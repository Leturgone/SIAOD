...
int double_fin_in_date(string& namebf, char date[], double stat) {
    ...
    //bf.seekg(0, ios::beg);
    //bf.read((char*)&r, sizeof(r));//читаем и сохраняем в r
    //while (numRecord < recordSize) {
    //    if (strcmp(r.v_date, date) == 0 && (r.stat_num == stat)) {
    //        flag1 = true;
    //        r.fine = r.fine * 2;
    //        bf.seekp(numRecord * sizeof(r), std::ios::beg);
    //        bf.write(reinterpret_cast<char*>(&r), sizeof(violation_record));
    //        /*bf.seekg((numRecord-1) * sizeof(r), std::ios::beg);*/
    //    }
    //    numRecord++;
    //    bf.read((char*)&r, sizeof(r));
    //}
